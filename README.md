...
### Индексы
todo размеры
| Таблица       | Индекс                                           | Пояснение                       |
|---------------|--------------------------------------------------|---------------------------------|
| UserLibrary   | idx_library_by_recency (user_id, last_played_at) | Отображение недавней активности |
| UserInventory | idx_inventory_by_user_game (user_id, game_od)    | Фильтрация по конкретной игре   |
| Game          | idx_games_active_by_developer (developer_id)     | Поиск игр по разработчику       |
| GameReview    | idx_game_review (game_id)                        | Отзывы по игре                  |
| TradeOffer    | idx_tradeoffer_from_user (from_user_id, status)  | Поиск исходящих предложений     |
| TradeOffer    | idx_tradeoffer_to_user (to_user_id, status)      | Поиск входящих предложений      |
...
| TradeOffer     | MongoDB    | Массивные объемы, быстрая вставка и выборка                         |
| TradeOfferItem | MongoDB    | Массовая запись                                                     |
