...
<img width="1956" height="1729" alt="steam physic" src="https://github.com/user-attachments/assets/a2f54adb-1b85-4e88-a63a-231c155a0db7" />
...
| TradeOffer    | idx_trade_offer_id (id)                                  | B-Tree          | 8 байт              | 7.59 млрд   | 60.72 GB        | Доступ по ID                                         |
| TradeOffer    | idx_trade_offer_from_user_id (from_user_id, created_at)  | B-Tree          | 16 байт             | 7.59 млрд   | 121.44 GB       | Просмотр исходящих предложений с сортировкой по дате |
| TradeOffer    | idx_trade_offer_to_user_id (to_user_id, created_at)      | B-Tree          | 16 байт             | 7.59 млрд   | 121.44 GB       | Просмотр входящих предложений с сортировкой по дате  |

### Шардирование и резервирование СУБД
#### Шардирование
Будем шардировать UserLibrary по ключу user_id. Тем самым снизим нагрузку на запись, уменьшим размер индексов. Выберем число шардов 4. Их с запасом хватит при увеличении одновременно играющих числа пользователей.

#### Партицирование
Будем партицировать TradeOffer по дате (помесячно).  
...
|---------------------|--------------------------------------|-------------------------------------|---------------------|-------------|-----------------|
| GameSearch          | games (game_id, title)               | Поиск игр в магазине по названию    | 20 байт (4+16)      | 114662      | 2.2932 MB       |
...
| Go            | Backend                  | Высокая скорость, билд под любую систему                        |
| ElectronJS    | Десктопное приложение    | Билд под любую систему, в основе браузер                        |
| MongoDB       | Хранилище данных         | Документоориентированная                                        |
| Cassandra     | Хранилище данных         |                                                                 |
...
| Prometheus    | Сбор метрик              | Сбор и хранение метрик, связка с Grafana                        |
| Grafana       | Визуализация метрик      | Кастомизируемые дашборды, связка с Prometheus                   |
