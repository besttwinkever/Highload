...
| User          | idx_user_username (username)                             | B-Tree          | 16 байт             | 982M        | 15.712 GB       | Быстрый поиск при входе в систему                    |
| User          | idx_user_email (email)                                   | B-Tree          | 32 байта            | 982M        | 31.424 GB       | Быстрый поиск при регистрация/сбросе пароля          |
| UserSession   | idx_session_id (id)                                      | B-Tree          | 8 байт              | 1.964 млрд  | 15.712 GB       | Уникальный идентификатор сессии                      |
| UserSession   | idx_session_token (token)                                | B-Tree          | 32 байт             | 1.964 млрд  | 62.848 GB       | Поиск сессии по токену                               |
| UserSession   | idx_session_user_id (user_id)                            | B-Tree          | 8 байт              | 1.964 млрд  | 15.712 GB       | Поиск всех активных сессий пользователя              |
...
| Game          | idx_game_developer_id (id)                               | B-Tree          | 4 байт              | 114662      | 458.65 KB       | Поиск игр разработчика                               |
...
| Item          | idx_item_game_id (game_id)                               | B-Tree          | 8 байт              | 5М          | 40 MB           | Поиск предметов в конкретной игре                    |
...
| MarketListing | idx_market_listing_id (seller_user_id)                   | B-Tree          | 8 байт              | 28М         | 224 MB          | Просмотр лотов продавца                              |
